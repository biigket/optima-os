...
import type { QuattroStockItem } from '@/types/stock';
...
export default function QuattroDetailPage() {
  ...
  if (!item || !form) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4">
        <p className="text-muted-foreground">ไม่พบข้อมูลเครื่อง Quattro</p>
  ...
  const Field = ({ label, value, editKey, mono }: { label: string; value: string; editKey?: keyof QuattroStockItem; mono?: boolean }) => (
    <div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">รายละเอียดเครื่อง Quattro</p>
          </div>
        </div>
