...
const TABLE_DEFS: TableDef[] = [
  ...
  {
    label: 'คลังสินค้า — ND2',
    table: 'qc_stock_items',
    columns: [
      { key: 'product_type', label: 'ประเภทสินค้า', required: true, example: 'ND2' },
      { key: 'serial_number', label: 'Serial Number', required: true, example: 'HNT-2024-001' },
      { key: 'status', label: 'สถานะ (พร้อมขาย/ติดจอง/ติดตั้งแล้ว/DEMO-ยืม/รอซ่อม-รอ QC/รอเคลม ตปท.)', example: 'พร้อมขาย' },
      { key: 'clinic', label: 'คลินิก', example: 'Beauty Clinic' },
      { key: 'hfl1', label: 'HFL1', example: 'FL45-001' },
      { key: 'hfl2', label: 'HFL2', example: 'FL30-001' },
      { key: 'hsd1', label: 'HSD1', example: 'SD45-001' },
      { key: 'hsd2', label: 'HSD2', example: 'SD30-001' },
      { key: 'hrm', label: 'HRM', example: 'RM-001' },
      { key: 'hrm_sell_or_keep', label: 'HRM ขาย/เก็บ', example: 'ขาย' },
      { key: 'ups_stabilizer', label: 'UPS/Stabilizer', example: 'APC-001' },
      { key: 'received_date', label: 'วันที่รับเข้า (YYYY-MM-DD)', example: '2024-06-01' },
      { key: 'storage_location', label: 'สถานที่เก็บ', example: 'คลัง A' },
      { key: 'inspection_doc', label: 'เอกสารตรวจสอบ', example: '' },
      { key: 'fail_reason', label: 'เหตุผลไม่ผ่าน QC', example: '' },
      { key: 'notes', label: 'หมายเหตุ', example: '' },
    ],
  },
  {
    label: 'คลังสินค้า — Trica 3D',
    table: 'qc_stock_items',
    columns: [
      { key: 'product_type', label: 'ประเภทสินค้า', required: true, example: 'TRICA3D' },
      { key: 'serial_number', label: 'Serial Number', required: true, example: 'TR3D-2024-001' },
      { key: 'status', label: 'สถานะ', example: 'พร้อมขาย' },
      { key: 'clinic', label: 'คลินิก', example: '' },
      { key: 'received_date', label: 'วันที่รับเข้า (YYYY-MM-DD)', example: '2024-06-01' },
      { key: 'install_date', label: 'วันติดตั้ง (YYYY-MM-DD)', example: '' },
      { key: 'borrow_from', label: 'ยืมจาก', example: '' },
      { key: 'borrow_to', label: 'ยืมให้', example: '' },
      { key: 'email_trica', label: 'Email Trica', example: '' },
      { key: 'storage_location', label: 'สถานที่เก็บ', example: 'คลัง A' },
      { key: 'fail_reason', label: 'เหตุผลไม่ผ่าน QC', example: '' },
      { key: 'notes', label: 'หมายเหตุ', example: '' },
    ],
  ...
  {
    label: 'คลังสินค้า — Quattro',
    table: 'qc_stock_items',
    columns: [
      { key: 'product_type', label: 'ประเภทสินค้า', required: true, example: 'QUATTRO' },
      { key: 'serial_number', label: 'Serial Number', required: true, example: 'QT-2024-001' },
      { key: 'handpiece', label: 'Handpiece', example: 'HP-001' },
      { key: 'status', label: 'สถานะ', example: 'พร้อมขาย' },
      { key: 'received_date', label: 'วันที่รับเข้า (YYYY-MM-DD)', example: '2024-06-01' },
      { key: 'storage_location', label: 'สถานที่เก็บ', example: 'คลัง A' },
      { key: 'fail_reason', label: 'เหตุผลไม่ผ่าน QC', example: '' },
  ...
  {
    label: 'คลังสินค้า — Picohi',
    table: 'qc_stock_items',
    columns: [
      { key: 'product_type', label: 'ประเภทสินค้า', required: true, example: 'PICOHI' },
      { key: 'serial_number', label: 'Serial Number', required: true, example: 'PH-2024-001' },
      { key: 'handpiece', label: 'Handpiece', example: 'HP-001' },
      { key: 'status', label: 'สถานะ', example: 'พร้อมขาย' },
      { key: 'received_date', label: 'วันที่รับเข้า (YYYY-MM-DD)', example: '2024-06-01' },
      { key: 'storage_location', label: 'สถานที่เก็บ', example: 'คลัง A' },
  ...
  {
    label: 'คลังสินค้า — Freezero',
    table: 'qc_stock_items',
    columns: [
      { key: 'product_type', label: 'ประเภทสินค้า', required: true, example: 'FREEZERO' },
      { key: 'serial_number', label: 'Serial Number', required: true, example: 'FZ-2024-001' },
      { key: 'handpiece', label: 'Handpiece', example: 'HP-001' },
      { key: 'status', label: 'สถานะ', example: 'พร้อมขาย' },
      { key: 'received_date', label: 'วันที่รับเข้า (YYYY-MM-DD)', example: '2024-06-01' },
      { key: 'storage_location', label: 'สถานที่เก็บ', example: 'คลัง A' },
  ...
  {
    label: 'คลังสินค้า — Cartridge',
    table: 'qc_stock_items',
    columns: [
      { key: 'product_type', label: 'ประเภทสินค้า', required: true, example: 'CARTRIDGE' },
      { key: 'serial_number', label: 'Serial Number', required: true, example: 'CRT-2024-001' },
      { key: 'cartridge_type', label: 'ประเภท Cartridge (A2.0/A3.0/A4.5/A6.0/L1.5/L3.0/L4.5/L9.0/N49/I49/N25/I25)', example: 'A4.5' },
      { key: 'status', label: 'สถานะ', example: 'พร้อมขาย' },
      { key: 'received_date', label: 'วันที่รับเข้า (YYYY-MM-DD)', example: '2024-06-01' },
      { key: 'storage_location', label: 'สถานที่เก็บ', example: 'คลัง A' },
      { key: 'fail_reason', label: 'เหตุผลไม่ผ่าน QC', example: '' },
