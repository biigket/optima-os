...
export default function PicohiDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const item = mockPicohiStock.find(i => i.id === id);
  const [editing, setEditing] = useState(false);
  const [form, setForm] = useState<PicohiStockItem | null>(item ?? null);

  if (!item || !form) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4">
        <p className="text-muted-foreground">ไม่พบข้อมูลเครื่อง Picohi</p>
        <Button variant="outline" onClick={() => navigate('/qc-stock')}>กลับ</Button>
      </div>
    );
  }

  const set = (key: keyof PicohiStockItem, value: string) => setForm(prev => prev ? { ...prev, [key]: value } : prev);

  const handleSave = () => {
    if (!form.serialNumber.trim()) { toast.error('กรุณากรอก S/N'); return; }
    const idx = mockPicohiStock.findIndex(i => i.id === id);
    if (idx !== -1) Object.assign(mockPicohiStock[idx], form);
    setEditing(false);
  ...
  const Field = ({ label, value, editKey, mono }: { label: string; value: string; editKey?: keyof PicohiStockItem; mono?: boolean }) => (
    <div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">รายละเอียดเครื่อง Picohi</p>
          </div>
        </div>
