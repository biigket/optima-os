...
export default function FreezeroDetailPage() {
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();
  const item = mockFreezeroStock.find(i => i.id === id);
  const [editing, setEditing] = useState(false);
  const [form, setForm] = useState<FreezeroStockItem | null>(item ?? null);

  if (!item || !form) {
    return (
      <div className="flex flex-col items-center justify-center py-20 space-y-4">
        <p className="text-muted-foreground">ไม่พบข้อมูลเครื่อง Freezero</p>
        <Button variant="outline" onClick={() => navigate('/qc-stock')}>กลับ</Button>
      </div>
    );
  }

  const set = (key: keyof FreezeroStockItem, value: string) => setForm(prev => prev ? { ...prev, [key]: value } : prev);

  const handleSave = () => {
    if (!form.serialNumber.trim()) { toast.error('กรุณากรอก S/N'); return; }
    const idx = mockFreezeroStock.findIndex(i => i.id === id);
    if (idx !== -1) Object.assign(mockFreezeroStock[idx], form);
    setEditing(false);
  ...
  const Field = ({ label, value, editKey, mono }: { label: string; value: string; editKey?: keyof FreezeroStockItem; mono?: boolean }) => (
    <div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">รายละเอียดเครื่อง Freezero</p>
          </div>
        </div>
      ...
      <div className="rounded-lg border bg-card p-6 space-y-6">
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="S/N" value={form.serialNumber} editKey="serialNumber" mono />
            <Field label="Handpiece" value={form.handpiece} editKey="handpiece" />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            {editing && (form.status === 'รอซ่อม/รอ QC' || form.status === 'รอเคลม ตปท.') && (
              <Field label="เครื่องเสียเพราะ?" value={form.failReason} editKey="failReason" />
            )}
          </div>
        </div>
